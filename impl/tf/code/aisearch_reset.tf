...
data "http" "ai_search_indexer_mmai_text_reset" {
  url    = "https://${local.ai_search_name}.search.windows.net/indexers/${jsondecode(restapi_object.ai_search_indexer_mmai_text.api_response).name}/reset?api-version=2024-05-01-preview"
  method = "POST"
  request_headers = {
...
# resource "null_resource" "ai_search_indexer_mmai_text_reset" {
#   provisioner "local-exec" {
#     command = <<EOT
#       curl -X POST "https://${local.ai_search_name}.search.windows.net/indexers/${jsondecode(restapi_object.ai_search_indexer_mmai_text.api_response).name}/reset?api-version=2024-07-01" \
