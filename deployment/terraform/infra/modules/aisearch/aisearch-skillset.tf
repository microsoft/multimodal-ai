  ...
  provisioner "local-exec" {
    ...
    command     = <<EOT
      ...
      az rest --method PUT ${local.line_separator}
        --url ${local.escape_char}"https://${var.search_service_name}.search.windows.net/skillsets${local.escape_char}(${local.escape_char}'${var.search_service_skillset_name}${local.escape_char}'${local.escape_char})?api-version=2024-11-01-preview${local.escape_char}" ${local.line_separator}
